...
const PartsOptionsForm = ({ onSubmit, formData }) => {
  ...
  useEffect(() => {
    const initialValues = {
      parts: formData.reduce((acc, { id }) => {
        acc[id] = {
          options: [{ name: '', price: '', quantity: '', image: null }]
        };
        return acc;
      }, {})
    };
  ...
  const onFinish = async (values) => {
    ...
    formattedData.forEach((data, index) => {
      ...
      payload.append(`option[${index}][quantity]`, data.quantity);
    });
                ...
                <div key={id}>
                  ...
                  <Form.List name={[id.toString(), 'options']}>
                    {(optionFields, { add: addOption, remove: removeOption }) => (
                      <>
                        {optionFields.map(({ key: optionKey, name: optionName, fieldKey: optionFieldKey, ...restOptionField }, index) => (
                          <div key={optionKey} style={{ marginBottom: 16 }}>
                            <Row gutter={16} align="middle">
                              ...
                              <Col span={8}>
                                <Item
                                  {...restOptionField}
                                  name={[optionName, 'quantity']}
                                  rules={[{ required: true, message: 'Missing option quantity' }]}
                                  style={{ marginBottom: 0 }}
                                >
                                  <Input type="number" placeholder="Option quantity" />
                                </Item>
                              </Col>
                              <Col span={6}>
                                <Item
